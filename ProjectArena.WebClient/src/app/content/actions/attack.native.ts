...
export function attackAction(actor: Actor, power: number, scope: number, target: IActor, startingTime: number): ChangeDefinition[] {
  ...
  return [{
    ...
    logs: undefined,
    action: () => {
      target.getNeighboursByScope(scope)
        .forEach(x => x.handleEffects(['damage', 'physical-damage'], newPower, false, 1, startingTime, actor));
    }
  }];
}
